...
void getPulseDefaultSink(void *data) 
{
    ...
    pulseaudio_context = pa_context_new(mainloop_api, "mled device list");
...
void *input_pulse(void *data) 
{
    ...
    // audio->format = 16; moved to init function

    //const int frag_size = frames * channels * audio->format / 8 * 2;    // we double this because of 
                                                                        // cpu performance issues with pulseaudio

    const int frag_size = frames * channels * 2;
    ...
    if (!(s = pa_simple_new(NULL, "mled", PA_STREAM_RECORD, audio->source, "audio for mled", &ss,
                            NULL, &pb, &error))) 
    {
    ...
    while (!audio->terminate) 
    {
        ...
        write_to_audio_buffers(buf, (audio_t *) data, frames);
